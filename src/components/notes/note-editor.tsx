    ...
    return (
        <div className="relative shrink-0">
            ...
            {isOpen && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 z-[99999] flex items-end sm:items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-150">
                    ...
                    <div className="relative z-10 w-full max-w-xs sm:max-w-sm bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/15 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[75vh] animate-in zoom-in-95 duration-150">
                        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-white/10 bg-zinc-50/50 dark:bg-zinc-800/50">
                            ...
                            <button
                                ...
                                className="p-1.5 rounded-lg hover:bg-zinc-200 dark:hover:bg-white/10 text-muted-foreground transition-colors cursor-pointer"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-2 space-y-1">
                            ...
                            {customFonts.length > 0 && (
                                <>
                                    <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground border-t border-zinc-100 dark:border-white/10 pt-2 mt-2">
                                    ...
                                    <div className="grid grid-cols-1 gap-1">
                                        {customFonts.map((font) => (
                                            ...
                                            >
                                                ...
                                                <span className="text-xs opacity-60 font-mono">AaBbCc</span>
                                            </button>
                                        ))}
                                    </div>
                                </>
...
function NoteAudioBlock({ block, removeBlock }: any) {
    ...
    useEffect(() => {
        ...
        if (currentFileUri && currentFileUri.startsWith('http://localhost/_capacitor_file_')) {
            currentFileUri = currentFileUri.replace('http://localhost/_capacitor_file_', 'file://');
        }
        setAudioUrl(currentFileUri || '');
    }, [fileData.url]);
        ...
        return (
            <div className="flex items-center gap-4 w-full cursor-pointer hover:bg-white/5 p-2 rounded-lg transition-colors group" onClick={() => setShowPlayer(true)}>
                ...
                <div className="flex flex-col flex-1 min-w-0">
                    ...
                    <span className="text-xs text-muted-foreground truncate uppercase">AUDIO</span>
                </div>
            </div>
        );
    }
...
function FileBlockRenderer({ block, idx, isFirst, isLast, moveBlock, removeBlock, onChange, noteId }: any) {
    ...
    let ext = (fileData.name?.split('.').pop() || '').toLowerCase();
    if (!ext || ext === fileData.name?.toLowerCase()) {
